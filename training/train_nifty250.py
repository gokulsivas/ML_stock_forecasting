...
def create_return_sequences(df, sequence_length=90):
    ...
    # ✅ ADDED: lag return features — directly teach the model momentum
    for lag in [1, 2, 3, 5, 10]:
        df[f'return_lag_{lag}'] = df['close_price'].pct_change().shift(lag)

    # ✅ ADDED: volume spike ratio — volume surge often precedes direction
    ...
    return np.array(Xseq, dtype=np.float32), np.array(yseq, dtype=np.float32), scaler, feature_cols
...
def train_large_model():
    ...
    trainer.train(
        ...
        best_save_path='saved_models/returns_model.pth',        # ✅ FIXED

...
# training/train.py  — replace your loss function

class DirectionalLoss(nn.Module):
    """MSE + penalty for getting direction wrong"""
    def __init__(self, alpha=0.3):
        super().__init__()
        self.alpha = alpha   # weight of directional penalty

    def forward(self, predictions, targets):
        mse = nn.functional.mse_loss(predictions, targets)

        # Penalize when predicted sign != actual sign
        pred_sign   = torch.sign(predictions)
        target_sign = torch.sign(targets)
        direction_wrong = (pred_sign != target_sign).float()
        directional_penalty = direction_wrong.mean()

        return mse + self.alpha * directional_penalty
...
class StockTrainer:
    ...
    def __init__(self, model, device, config):
        ...
        self.criterion = DirectionalLoss(alpha=0.5)          # ✅ FIXED: was 0.7
        self.optimizer = torch.optim.AdamW(                  # ✅ IMPROVED: AdamW
